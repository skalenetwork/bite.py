...
async def encrypt_transaction(tx: Dict[str, str], endpoint: str) -> Dict[str, str]:
    ...
    try:
        ...
        encrypted_data = encrypt_message(rlp_encoded_data, endpoint)
        ...
        logger.logger.error(f'Error encrypting transaction: {error}')
        raise
...
async def encrypt_transaction_mockup(tx: Dict[str, str]) -> Dict[str, str]:
    ...
    try:
        ...
        logger.logger.error(f'Error encrypting transaction (mockup): {error}')
        raise
...
async def encrypt_message(message: str, endpoint: str) -> str:
    ...
    try:
        ...
        public_key_responses = await bite_rpc.get_committees_info(endpoint)
        
        if len(public_key_responses) == 1:
            public_key_response = public_key_responses[0]
            encrypted_raw_message = await t_encrypt_adapter.encrypt_message(
                data, 
            ...
            encrypted_raw_message = await t_encrypt_adapter.encrypt_message_dual_key(
                data,
        ...
        logger.logger.error(f'Error encrypting message: {error}')
        raise
...
def encrypt_message_mockup(message: str) -> str:
    ...
    try:
        ...
        encrypted_raw_message = t_encrypt_adapter.encrypt_message_mockup(data)
        epoch_id = 0
...
def _validate_and_extract_transaction_fields(tx: Dict[str, str]) -> Dict[str, str]:
    ...
    if not is_valid:
        raise ValueError("Invalid input: Must be an object with 'data' and 'to' fields of type string")
    ...
    if len(tx_to) != 40:
        raise ValueError("Invalid input: 'to' field must be exactly 20 bytes (40 hex characters) long")
...
def _rlp_encode_transaction_data(tx_to: str, tx_data: str) -> str:
    ...
    try:
        ...
        logger.logger.error(f'Error RLP encoding transaction data: {error}')
        raise Exception('Failed to RLP encode transaction data')
...
def _rlp_encode_message_data(data: list) -> str:
    ...
    try:
        ...
        logger.logger.error(f'Error RLP encoding message data: {error}')
        raise Exception('Failed to RLP encode message data')
