...
def get_decrypted_transaction_data(endpoint: str, transaction_hash: str) -> str:
    ...
    try:
        ...
        logger.logger.error(f'Error fetching decrypted transaction data: {error}')
        raise
    ...
    try:
        ...
        for item in result:
            ...
            if not re.match(r'^[0-9a-fA-F]{256}$', item['commonBLSPublicKey']):
                raise ValueError('commonBLSPublicKey is not a valid 256-character hexadecimal string')
        ...
        logger.logger.error(f'Error fetching BITE common public key: {error}')
        raise
...
def _send_rpc_request(endpoint: str, request_body: Dict[str, Any]) -> Any:
    ...
    try:
        ...
        if 'error' in data:
            raise Exception(f"Error from server: {data['error']['message']}")
        
        return data.get('result')
    except Exception as error:
        logger.logger.error(f'Error sending RPC request: {error}')
        raise
